...
if [[ $(uname) = "Linux" ]]; then
	...
	nm -un pipex | awk '{print $2}' | sed 's/@.*//' | grep -F -f fn_allowed | sort | uniq > fn_used
else
	nm -un pipex | grep -v __ | cut -c2- > fn_used
fi

# Ensure no extra whitespace in fn_used
sed -i 's/^[ \t]*//;s/[ \t]*$//' fn_used
sed -i 's/^[ \t]*//;s/[ \t]*$//' fn_allowed

# Filter fn_used to only include allowed functions
grep -F -f fn_allowed fn_used > check

# Ensure no extra whitespace in check
sed -i 's/^[ \t]*//;s/[ \t]*$//' check

# Debugging: Print contents of fn_used, fn_allowed, and check
echo 'Contents of fn_used:'
cat fn_used
echo 'Contents of fn_allowed:'
cat fn_allowed
echo 'Contents of check:'
cat check
